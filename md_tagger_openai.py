#!/usr/bin/env python3
"""
마크다운 파일 태그 자동 생성기 (OpenAI 버전)

OpenAI의 gpt-4.1-mini 모델을 사용하여 마크다운 파일을 분석하고
...
class MarkdownTagger:
    def __init__(self, model_name: str = "gpt-4.1-mini", max_tokens: int = 32768, chunk_size: int = 30000):
        """
        마크다운 태거 초기화
        ...
        if langsmith_api_key:
            ...
            os.environ["LANGSMITH_PROJECT"] = os.getenv("LANGSMITH_PROJECT", "md-tagger")
    ...
    @traceable(name="generate_tags_for_chunk")
    def generate_tags_for_chunk(self, chunk: str) -> List[str]:
        """
        청크에 대해 OpenAI API를 호출하여 태그를 생성합니다.
        """
...
파이썬프로그래밍"""
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "당신은 한국어 텍스트를 분석하여 적절한 해시태그를 생성하는 AI입니다."},
    ...
    def process_markdown_file(self, file_path: str) -> bool:
        ...
        """
        print(f"\n파일 처리 시작: {file_path} (모델: {self.model_name})")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='OpenAI를 사용하여 마크다운 파일에 자동으로 태그를 추가합니다.'
    )
    parser.add_argument(
        'path',
    ...
    parser.add_argument(
        ...
        help='사용할 OpenAI 모델 (기본값: gpt-4.1-mini)'
    )
    parser.add_argument(
        '--max-tokens',
        type=int,
